...
class WdaClient::SessionsTest < Minitest::Test
  def test_install_and_get_session
    ...
    caps =<<-CAPS
{
  ...
  def test_install_and_get_session_without_app
    base_host = 'localhost:8100'
    ...
    caps =<<-CAPS
{
  "desiredCapabilities": {
    ...
    stub_request(:get, "#{base_host}/session/26FA8CEA-9D59-4EB1-8B19-84AFD7307936")
      .with(headers:{ 'Content-Type' => 'application/json' })
      .to_return(body: json)
    ...
    res_json_body = client.get_current_session
    assert_equal JSON.parse(json), res_json_body
    assert_equal "26FA8CEA-9D59-4EB1-8B19-84AFD7307936", client.session_id
    assert_equal 0, client.status
  end
